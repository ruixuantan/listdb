...
void PmemMgr::StartThreads() {
  bg_thread_ = std::thread(std::bind(&PmemMgr::BackgroundThreadLoop, this));

  for (int i = 0; i < kNumWorkers; ++i) {
    worker_data_[i].id = i;
    worker_data_[i].stop = false;
    worker_threads_[i] = std::thread(std::bind(
        &PmemMgr::CompactionWorkerThreadLoop, this, &worker_data_[i]));
  }
}

void PmemMgr::BackgroundThreadLoop() {
  std::cout << "Background thread loop" << std::endl;
}

void PmemMgr::CompactionWorkerThreadLoop(CompactionWorkerData* td) {
